...
class ClCoupon {
  Future<bool> isHaveCouponOrStamp(context) async {
    List<StampModel> stamps = await loadStamps(context, {
      'user_id': globals.userId,
      'use_flag': '1',
    });
    if (stamps.isNotEmpty) return true;

    List<CouponModel> coupons = await loadCoupons(context, {
      'user_id': globals.userId,
      'use_flag': '1',
    });
  ...
  Future<RankModel> loadRankData(context, userId) async {
    ...
    await Webservice()
        ...
        .then((v) => {results = v});
    RankModel rank = RankModel.fromJson(results['rank']);
    return rank;
  }
  ...
  Future<List<StampModel>> loadUserStamps(context, userId) async {
    ...
    await Webservice()
        .loadHttp(context, apiLoadUserStampUrl, {
          'user_id': userId,
          'company_id': APPCOMANYID,
        })
  ...
  Future<int> loadDayStampCount(context, userId) async {
    Map<dynamic, dynamic> stampResults = {};
    await Webservice()
        .loadHttp(context, apiLoadDayStampCntUrl, {'user_id': userId})
        .then((value) => stampResults = value);
