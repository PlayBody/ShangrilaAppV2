...
class ClUser {
  Future<UserModel?> getUserModel(context, dynamic condition) async {
    ...
    await Webservice()
        .loadHttp(context, apiUrl, {'condition': jsonEncode(condition)})
        .then((v) => {results = v});
    if (kDebugMode) {
      print(results);
    }
  ...
  Future<UserModel> getUserFromId(context, userId) async {
    ...
    Map<dynamic, dynamic> results = {};
    await Webservice()
        .loadHttp(context, apiUrl, {'user_id': userId})
        .then((v) => {results = v});
  ...
  Future<List<TicketModel>> loadUserTickets(context) async {
    ...
    Map<dynamic, dynamic> results = {};
    await Webservice()
        .loadHttp(context, apiUrl, {'user_id': globals.userId})
        .then((v) => {results = v});
  ...
  Future<bool> sendResetEmail(context, String email) async {
    ...
    Map<dynamic, dynamic> results = {};
    await Webservice()
        .loadHttp(context, apiUrl, {'company_id': APPCOMANYID, 'email': email})
  ...
  Future<bool> deleteUser(context, userId) async {
    await Webservice().loadHttp(context, apiDeleteUserInfoUrl, {
      'user_id': userId,
    });
