...
class _ConnectOrganList extends State<ConnectOrganList> {
  ...
  Future<List> loadInitData() async {
    ...
    initCameraPosition = CameraPosition(target: initialPosition, zoom: 14);
    setState(() {});
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: item.organImage == null || item.organImage!.isEmpty
            ? Image.network(
                '${organImageUrl}no-organ-image-company-1.png',
                fit: BoxFit.contain,
              )
            : Image.network(
                organImageUrl + item.organImage!,
                fit: BoxFit.cover,
              ),
      ),
    );
              ...
              children: [
                ...
                if (item.snsurl != null && item.snsurl != '')
                  ElevatedButton(
                    style: ElevatedButton.styleFrom(
                      elevation: 0,
                      backgroundColor: Color(0xff4db5da),
                      padding: EdgeInsets.all(6),
                      visualDensity: VisualDensity(vertical: -3, horizontal: 3),
                    ),
